...
const CriteriaConfig: React.FC<CriteriaConfigProps> = ({
  ...
  return (
    <>
      {" "}
      {/* Menggunakan Fragment karena section sudah ada di parent */}
      <h2 className="text-2xl font-semibold mb-4">3. Konfigurasi Kriteria</h2>
        ...
        {criteria.map((crit) => (
          <div
            ...
            <select
              ...
              onChange={(e) =>
                onCriteriaTypeChange(crit, e.target.value as CriteriaType)
              }
      ...
      <p className="mb-4 text-sm text-gray-600">
        <small>
          ...
          penting. Gunakan nama kriteria yang sama persis seperti di data Anda.
        </small>
      </p>
      <div className="flex flex-col gap-3">
        {prioritizedCriteria.map((_crit, index) => (
          <div key={index} className="flex items-center gap-3">
            <span className="font-bold text-gray-700">Rank {index + 1}:</span>
            <select
